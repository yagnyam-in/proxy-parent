...
import in.yagnyam.proxy.utils.Errors;
...
public class AuthenticationTokenService {
  ...
  private AuthenticationToken jwtContextToAuthenticationToken(@NonNull JwtContext jwtContext)
    ...
    claims.getClaimNames().forEach(Errors.rethrow().wrap(claimName -> {
      if (claims.isClaimValueString(claimName)) {
        ...
        tokenBuilder.dateAttribute(claimName,
            new Date(claims.getNumericDateClaimValue(claimName).getValueInMillis()));
      }
    }));
  ...
  private String extractKeyId(@NonNull JwtContext jwtContext) throws InvalidJwtException {
    return jwtContext.getJoseObjects().stream()
        ...
        .orElseThrow(() -> invalidJwtException(ErrorCodes.MISCELLANEOUS, KEY_ID + " is missing",
            jwtContext));
  }

  private InvalidJwtException invalidJwtException(int errorCode, String errorMessage,
