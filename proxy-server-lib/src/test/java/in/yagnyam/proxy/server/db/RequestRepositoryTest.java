...
public class RequestRepositoryTest extends RepositoryTestHelper {
  ...
  public void testGetRequest_Valid() {
    RequestEntity requestEntity = RequestEntity.builder().requestId("RID").requestType("RT")
        .build();
  ...
  public void testSaveRequest_Valid() {
    ...
    assertEquals(requestEntity,
        ObjectifyService.run(() -> ofy().load().key(Key.create(RequestEntity.class, "RID")).now()));
  }
