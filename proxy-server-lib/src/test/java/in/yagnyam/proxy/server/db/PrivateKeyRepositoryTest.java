...
public class PrivateKeyRepositoryTest extends RepositoryTestHelper {
  ...
  public void testGetIssuer_Valid() {
    ...
    assertEquals(privateKeyEntity,
        issuerRepository.getPrivateKeyEntity(privateKeyEntity.getId()).get());
  }
  ...
  public void testSaveIssuer_Valid() {
    ...
    assertEquals(privateKeyEntity, ObjectifyService
        .run(() -> ofy().load().key(Key.create(PrivateKeyEntity.class, "3")).now()));
  }
}
