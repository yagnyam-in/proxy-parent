...
public class PrivateKeyService {
  ...
  @Builder
  public PrivateKeyService(@NonNull PrivateKeyRepository privateKeyRepository,
  ...
  private PrivateKeyEntity enrichPrivateEntity(PrivateKeyEntity privateKeyEntity) {
    try {
      if (StringUtils.isValid(privateKeyEntity.getCertificateEncoded())) {
        privateKeyEntity
            .setCertificate(pemService.decodeCertificate(privateKeyEntity.getCertificateEncoded()));
      }
      if (StringUtils.isValid(privateKeyEntity.getPrivateKeyEncoded())) {
        privateKeyEntity.setPrivateKey(pemService
            .decodePrivateKey(privateKeyEntity.getPrivateKeyAlgorithm(),
                privateKeyEntity.getPrivateKeyEncoded()));
      }
